...
// GET - Listar agendamentos de consulta por pet
export async function GET(request: NextRequest) {
  try {
    const petId = request.nextUrl.searchParams.get('pet_id');
    const status = request.nextUrl.searchParams.get('status');

    let query = supabase.from('consultation_appointments').select('*');
...
// POST - Criar agendamento de consulta
...
// PUT - Atualizar agendamento de consulta
